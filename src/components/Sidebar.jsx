...
export default function Sidebar({ activeTab, setActiveTab, openAnalysis }) {
  
  const handleTabClick = (tabLabel) => {
    if (tabLabel === "Skin Analysis") {
      openAnalysis(); // This opens the pop-up
    } else {
  ...
  return (
    <aside className="w-72 bg-white border-r border-slate-100 hidden lg:flex flex-col sticky top-0 h-screen shadow-sm">
      <div className="p-8">
...
function NavItem({ icon, label, active, onClick, color = "text-slate-500" }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-bold transition-all duration-200 ${
      active ? "bg-blue-600 text-white shadow-lg shadow-blue-100 scale-[1.02]" : `${color} hover:bg-slate-50 hover:text-slate-900`
    }`}>
      {icon}
